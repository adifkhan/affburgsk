...
import { PayoutHistoryData } from '@/types/models';

type PayoutHistoryProps = {
    rowsAllData: PayoutHistoryData[]
}
...
export default function PayoutHistoryTable({ rowsAllData }: PayoutHistoryProps) {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = rowsAllData.map((n) => parseInt(n.id));
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - rowsAllData.length) : 0;

    const visibleRows = React.useMemo(
        () =>
            stableSort(rowsAllData, getComparator(order, orderBy)).slice(
                page * rowsPerPage,
                page * rowsPerPage + rowsPerPage,
            ),
        [order, orderBy, page, rowsAllData, rowsPerPage],
    );
