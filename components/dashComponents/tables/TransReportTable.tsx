...
import { MainReportData, PayoutHistoryData, TransactionDataType } from '@/types/models';

type TransReportProps = {
    transReportAllData: MainReportData[]
}
...
interface EnhancedTableProps {
    numSelected: number;
    onRequestSort: (event: React.MouseEvent<unknown>, property: keyof PayoutHistoryData) => void;
...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler =
        (property: keyof PayoutHistoryData) => (event: React.MouseEvent<unknown>) => {
...
export default function MainReportTable({ transReportAllData }: TransReportProps) {
    const [order, setOrder] = React.useState<Order>('asc');
    const [orderBy, setOrderBy] = React.useState<keyof PayoutHistoryData>('Date');
    ...
    const handleRequestSort = (
        ...
        property: keyof PayoutHistoryData,
    ) => {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = transReportAllData.map((n) => parseInt(n.id));
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - transReportAllData.length) : 0;

    const visibleRows = React.useMemo(
        () =>
            stableSort(transReportAllData, getComparator(order, orderBy)).slice(
                page * rowsPerPage,
                page * rowsPerPage + rowsPerPage,
            ),
        [order, orderBy, page, transReportAllData, rowsPerPage],
    );

    return (
        <Box sx={{ width: '100%', }}>
            <Paper sx={{ width: '100%', mb: 2, backgroundColor: '#383B8C', borderRadius: '8px 8px 0 0', color: 'lightgrey', fontFamily: 'Dosis' }}>
                ...
                <TableContainer>
                    ...
                    >
                        <EnhancedTableHead
                            ...
                            rowCount={transReportAllData.length}
                        />
                ...
                <TablePagination
                    sx={{
                        color: 'lightGrey', bgcolor: '#1c2437', borderRadius: '0 0 0 0',
                        '.MuiSvgIcon-root': {
                            fill: 'lightGrey',
                        },
                        '.Mui-disabled': {
                    ...
                    component="div"
                    count={transReportAllData.length}
