...
import { MainReportData, PayoutHistoryData } from '@/types/models';

type MainReportProps = {
    mainReportAllData: MainReportData[]
}
...
export default function MainReportTable({ mainReportAllData }: MainReportProps) {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = mainReportAllData.map((n) => parseInt(n.id));
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - mainReportAllData.length) : 0;

    const visibleRows = React.useMemo(
        () =>
            stableSort(mainReportAllData, getComparator(order, orderBy)).slice(
                page * rowsPerPage,
                page * rowsPerPage + rowsPerPage,
            ),
        [order, orderBy, page, mainReportAllData, rowsPerPage],
    );
        ...
        <Box sx={{ width: '100%', }}>
            <Paper sx={{ width: '100%', mb: 2, backgroundColor: '#383B8C', borderRadius: '8px 8px 9px 9px', color: 'lightgrey', fontFamily: 'Dosis' }}>
                ...
                <TableContainer>
                    ...
                    >
                        <EnhancedTableHead
                            ...
                            rowCount={mainReportAllData.length}
                        />
                ...
                <TablePagination
                    ...
                    component="div"
                    count={mainReportAllData.length}
